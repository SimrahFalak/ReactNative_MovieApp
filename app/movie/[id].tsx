  ...
  return (
    <View className='bg-primary flex-1'>
      <ScrollView contentContainerStyle={{ paddingBottom:80 }}>
        ...
        <View className="px-5 mt-5 flex flex-col items-start justify-center">
          <Text className="text-white text-xl font-bold">{movie?.title}</Text>
          <View className='flex flex-row items-center gap-x-1 mt-2'>
          <Text className="text-light-200 text-sm">{movie?.release_date?.split('-')[0]}</Text>
          <Text className="text-light-200 text-sm">{movie?.runtime}m</Text>
          </View>
          <View className='flex flex-row items-center px-2 py-1 rounded-md bg-dark-200 mt-2 gap-x-1'>
